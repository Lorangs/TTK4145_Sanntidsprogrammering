...
Condition var e veldig lav level, du må gjær alt skjøl veldig manuelt, so det e lett at det blir mennesklige feil.
Java er veldig høy level, der har alt innebygd condition var og lås
Ada e tydelig med å bruke enterys med guards så e lett å lese når du kan gjer ulike ting
...
- Bugs in this kind of low-level synchronization can be hard to spot.
  ...
  I am most confident in the ada method, i think it is becouse it is straight forward and does not allow entery when the guards are not furfilled, so codecuallety whise you can simely read the guards ang understand what happens
...
- We operated only with two priority levels here, but it makes sense for this "kind" of priority resource to support more priorities.
  ...
  When using the priorety queue this coud probably be implemented quide simply wich means good code quality
  I also think the ada code coud be modified witout to much truble by putting in guards, but for large numbers it bight become messy
  The other solutions woud be tricier i think, which reduse code quality
...
Der fins so mange beire alternativ til get value at eg ser ikkje poeng med å bruke den.
tvilsom fordi: får fort race conditions, kan ikkje bruke det på alle platforma og e veldig lavlevel og masse å holde styr på.
