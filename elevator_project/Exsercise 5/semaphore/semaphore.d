...
class Resource(T)
{
    ...
    {
        mtx.wait(); // we have practecally made a mutex using Semaphores, this results in safer handeling of variables
        if (busy) {
            numWaiting[priority]++; //we make shure the tread knows it is wating
            mtx.notify(); //release the mutex
            sems[priority].wait(); //it implicitly reacquires the mutex
            numWaiting[priority]--; // so it can edit the sheared variables
        }
        busy = true; 
        mtx.notify(); //then it realises the mutex
        return value; 
    }
    ...
    void deallocate(T v)
    {
        ...
        // high-priority
        if (numWaiting[1] > 0) { //num wating is safer thar get value becouse it is set before allocate drops the mutex, and decresed after it si done waiting.
            sems[1].notify(); 
        }
        // low-priority
        ...
        // If no waiters, release the mutex
        else{
        	mtx.notify();
        }
    }
