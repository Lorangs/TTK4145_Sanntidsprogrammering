...
func resourceManager(takeLow chan Resource, takeHigh chan Resource, giveBack chan Resource) {
	...
	for {
		// Try to send the resource to a high-priority user first
		select {
		case takeHigh <- res:
			// Resource taken by high-priority user
			res = <-giveBack
			continue
		default:
			// No high-priority user is ready, try low-priority
		}

		// Try to send the resource to a low-priority user
		select {
		case takeHigh <- res:
			// Resource taken by high-priority user
			res = <-giveBack
		case takeLow <- res:
			// Resource taken by low-priority user
			res = <-giveBack
		case res = <-giveBack:
			// Resource returned by a user
