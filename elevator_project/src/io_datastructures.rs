use crate::config::{NUMBER_OF_ELEVATORS, NUMBER_OF_FLOORS};
...
/// Used as a container by master and backup to keep track of current requests and slave states.
...
impl OrderRequests {
    ...
    pub fn init() -> OrderRequests {
        ...
        let states: [ElevatorState; NUMBER_OF_ELEVATORS] =
            [ElevatorState::init(); NUMBER_OF_ELEVATORS];
    ...
    /// Run the optimization algorithm to get the next orders for all slaves.
    /// Then return the closest order for the given slave.
    pub fn get_next_order(&mut self, slave_number: usize) -> Result<Option<CallButton>, Error> {
        ...
        if elevator.behaviour != ElevatorBehaviour::OutOfOrder {
            ...
            // Lambda function to check and create call button
