...
/// Spawn a new thread that will sleep for the given duration and then send a message to the channel when done.
pub fn start_timer(tx: cbc::Sender<bool>, duration_s: u64) {
    spawn(move || {
        sleep(Duration::from_secs(duration_s));
        tx.send(true).unwrap();
    });
}
