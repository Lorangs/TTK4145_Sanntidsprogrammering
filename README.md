...
## How to run the thing - start_script.sh

Dette skriptet bygger og kjører de ulike delene av heissystemet samtidig. Det åpner flere terminaler:  
- Én for master_main  
- Én for backup_main  
- Flere par av SimElevatorServer og slave_main (avhengig av antall slaver)

## Hvordan kjøre skriptet

```bash
./start_script.sh [FLAGG]
```

**Flagg:**
- `-p <port>`: Startport for heissimulator (første heis har standardverdi: 9000)  
- `-r`: Bruker `--release` når du bygger/kjører med Cargo  
- `-n <antall_slaver>`: Antall slaver (standardverdi: 3)  
- `-f <antall_etasjer>`: Antall etasjer (standardverdi: 4)

Eksempel på kjøring:
```bash
./start_script.sh -p 9500 -n 2 -f 5
```

Skriptet vil da:
1. Kjøre `cargo build` uten `--release`.   
2. Kjøre master_main og backup_main i to separate terminaler.  
3. Opprette 2 grupper med SimElevatorServer og tilhørende slave_main (totalt 2 * 2 = 4 terminaler). Med startport 9500 og 5 etasjer.
4. For hver ny slave, økes X-posisjonen for terminalene, slik at de ikke overlapper.  
